...
function weatherBycity()
{
  ...
  url='https://api.openweathermap.org/data/2.5/weather?q='+search_city+'&appid='+key;
  getWeather(url);
}
function weatherBylocation()
{
  if (navigator.geolocation) {
    ...
    document.getElementById("weather_main").innerHTML = "Geolocation is not supported by this browser.";
  }
}
...
fetch(url)  
    .then(function(response) {  
        if (response.status == 404) {
            document.getElementById("weather_main").innerHTML = "City not found. Please enter valid City name.";
            return false;    
        }
    ...
    console.log(data);
      ...
      let wind_direction;
      if(data.wind.deg>0 && data.wind.deg<=11.25)
      wind_direction = 'N';
      ...
      //temprature
      document.getElementById("temp").innerHTML = Math.round(parseFloat(data.main.temp)-273.15)+"&#8451;";
      document.getElementById("wind").innerHTML = data.wind.speed +" m/s " + "& Direction of wind :  " + wind_direction ;
      document.getElementById("cloud").innerHTML = data.clouds.all+"% cloudy";
      document.getElementById("sunRise").innerHTML = "Sun-Rise: "+ date_sunrise;
