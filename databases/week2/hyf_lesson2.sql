...
--Get all the tasks assigned to users whose email ends in @spotify.com.....ex-1
SELECT task.* from (
(task INNER JOIN user_task ON task.id=user_task.task_id)
INNER JOIN user ON user_task.user_id=user.id)
WHERE user.email LIKE '%@spotify.com';

-- or this way
SELECT task.* from task, user_task, user WHERE 
task.id=user_task.task_id AND 
user_task.user_id=user.id AND 
user.email LIKE '%@spotify.com';

--Get all the tasks for 'Donald Duck' with status 'Not started'....ex-2
SELECT * from task WHERE id IN 
(
SELECT task_id from user_task WHERE user_id IN (
SELECT id from user WHERE name = 'Donald Duck')
 ) AND
  status_id IN
 (SELECT id from status WHERE name = 'Not started');

 --or this way
 SELECT task.* from task, status, user_task, user WHERE
task.id = user_task.task_id AND
user_task.user_id = user.id AND
task.status_id = status.id AND
status.name = 'Not started' AND
user.name = 'Donald Duck';

--Get all the tasks for 'Maryrose Meadows' that were created in september (hint: month(created)=month_number)......ex-3
...
--Find how many tasks where created in each month, e.g. how many tasks were created in october, how many tasks were created in november, etc. (hint: use group by)....ex-4
